...
// 节流
export function throttle (fn, delay = 30, { leading = true, trailing = true } = {}) {
  let ctx
  let args
  let timeout
  let previous = 0
...
// 防抖
export function debounce (fun, delay = 30) {
...
// 千分隔数求和
export function milliPlus (nums) {
  let value = 0
  const n = 10000000
  nums.forEach(num => {
    value = (Number(num.replace(/,/g, '')) * n + value * n) / n
  })
  return value
}
...
export function digitalToUppercase (n) {
  ...
  for (let i = 0; i < fraction.length; i++) {
    s += (digit[Math.floor(+(n * 10 * Math.pow(10, i)).toFixed(2)) % 10] + fraction[i]).replace(
      /零./,
      ''
    )
  }
...
// 失败提示弹窗
export const err = throttle(msg => Message.error(msg), 3000, { trailing: false })
